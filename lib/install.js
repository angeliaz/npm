...
function generateActionsToTake (installState, cb) {
  ...
  chain([
    ...
    [diffTrees, currentTree, idealTree, todo, log.newGroup("diffTrees")],
    ...
    [debugActions, "decomposeActions", todo]
  ], cb)
}
...
function normalizeTree (tree, cb) {
...
function printInstalled (installState, where, cb) {
  ...
  */
  differences.forEach(function (action) {
    if (action[0] !== "install") return
    var child = action[1]
    console.log(child.package.name+"@"+child.package.version + " " + path.relative(where, child.path))
  })
  log.showProgress()
  cb()
}

function debugActions (name, actionsToLog, cb) {
